...
template <class T>
array4D<T> parseConvWeights2array( std::string filename ) {
	...
	array4D<T> arr( { commaCount, commaCount, depthCount, amountCount } );
...
template <class T>
array1D<T> parseBias2array(std::string filename) {
...
template <class T>
array2D<T> parseFcWeights2array(std::string filename) {
	std::ifstream file; // stream for input file
	std::string textLine; // string for content
